...
!(async () => {
  $.newShareCodes = []
  $.authorCode = await getAuthorShareCode('https://raw.fastgit.org/a592345029/backup/main/ljd.txt')
  if (!$.authorCode) {
    $.http.get({url: 'https://raw.fastgit.org/a592345029/backup/main/ljd.txt'}).then((resp) => {}).catch((e) => $.log('刷新CDN异常', e));
    await $.wait(1000)
    $.authorCode = await getAuthorShareCode('https://raw.fastgit.org/a592345029/backup/main/ljd.txt') || []
  }
  if (!cookiesArr[0]) {

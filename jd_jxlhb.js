...
!(async () => {
  ...
  console.log('京喜领88元红包\n' +
      ...
      '温馨提示：如提示助力火爆，可尝试寻找京东客服')
  let res = []
  res = await getAuthorShareCode('https://raw.fastgit.org/a592345029/backup/main/lhb.txt')
  if (!res) {
    $.http.get({url: 'https://raw.fastgit.org/a592345029/backup/main/lhb.txt'}).then((resp) => {}).catch((e) => $.log('刷新CDN异常', e));
    await $.wait(1000)
    res = await getAuthorShareCode('https://raw.fastgit.org/a592345029/backup/main/lhb.txt')
  }
